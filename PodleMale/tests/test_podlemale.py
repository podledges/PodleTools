...
class ProtocolTests(unittest.TestCase):
    def test_protocol_tokens_are_versioned_ack_hello(self) -> None:
        ...
        self.assertEqual(DEFAULT_PORT, 42067)
        self.assertNotEqual(HELLO, b"BOT-PORTDRIC/1 HELLO\n")
    ...
    def test_cli_completes_ack_hello(self) -> None:
        ...
        client = subprocess.run(
            [str(ROOT / "bin" / "podlemale"), "hello", "--port", port],
            check=False,
            capture_output=True,
            text=True,
            timeout=5,
    ...
    def test_invalid_lines_are_ignored_until_hello(self) -> None:
        command = [
            str(ROOT / "bin" / "podlemale"),
            "listen",
            "--port",
        ...
        with socket.create_connection(("127.0.0.1", port), timeout=2) as junk:
            junk.sendall(b"NOPE\n")

        client = subprocess.run(
            [str(ROOT / "bin" / "podlemale"), "hello", "--port", str(port)],
            check=False,
            capture_output=True,
            text=True,
            timeout=5,
    ...
    def test_locked_tx_port_round_trip(self) -> None:
        command = [
            str(ROOT / "bin" / "podlemale"),
            "listen",
            "--once",
        ...
        assert server.stdout is not None
        listening = server.stdout.readline().strip()
        self.assertEqual(listening, "listening on 127.0.0.1:42067")

        client = subprocess.run(
            [str(ROOT / "bin" / "podlemale"), "hello"],
            check=False,
            capture_output=True,
            text=True,
            timeout=5,
