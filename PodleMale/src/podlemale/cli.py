"""Command-line interface for the Nix PodleMale TX hook."""
...
def require_loopback(value: str) -> str:
    """Reject names and addresses that could expose the hook to a network."""
    if value == "localhost":
        return DEFAULT_HOST
    try:
...
def send_hello(host: str, port: int, timeout: float) -> None:
    ...
    with socket.socket(family, socket.SOCK_STREAM) as connection:
        ...
        if _receive_line(connection) != ACK_HELLO:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="podlemale",
        description=(
            "Nix TX hook: send PORT-NIXVM/1 HELLO on loopback toward "
            "Windows Female (default 127.0.0.1:42067)."
        ),
    )
    ...
    listen_parser = subparsers.add_parser(
        "listen",
        help="listen for HELLO and reply ACK-HELLO (local stand-in when Windows is down)",
    )
    ...
    hello_parser = subparsers.add_parser(
        "hello", help="send PORT-NIXVM/1 HELLO and require ACK-HELLO"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        sys.stdout.write(ACK_HELLO.decode("utf-8"))
        return 0
    except (HandshakeError, OSError, ValueError) as exc:
        print(f"podlemale: {exc}", file=sys.stderr)
        return 1
