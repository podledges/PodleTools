"""Command-line interface for the Nix PodleFemale RX hook."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="podlefemale",
        description=(
            "Nix RX hook: listen for PORT-NIXVM/1 HELLO on loopback from "
            "Windows Male (default 127.0.0.1:46720)."
        ),
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    listen_parser = subparsers.add_parser(
        "listen", help="listen for HELLO and reply ACK-HELLO"
    )
    ...
    hello_parser = subparsers.add_parser(
        "hello",
        help="send PORT-NIXVM/1 HELLO and require ACK-HELLO (Nix-only proof)",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"podlefemale: {exc}", file=sys.stderr)
        return 1
